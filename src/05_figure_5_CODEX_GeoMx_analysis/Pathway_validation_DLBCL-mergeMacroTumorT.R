...
library(readxl)
source(file.path(loadingfunction_wdpath, "CODEX_GeoMX analysis for DLBCL","1-Pathway_validation_DLBCL_preload_function.R"))
### process codex data
...
# read in object
spe_obj <- qread(file.path(wdpath,"GeoMx_count_table","spe_for_DLBCL_mergeTumorTmacro.qs"))
dim(spe_obj)
...
# qsave(Upper_seurat_list,file = file.path(wdpath,"GeoMX_batchcorrection","upperquartile-Seurat_for_DLBCL_includeTumorMergeMacro-batchcorrected-DEGvalidated.qs"))
...
# qsave(LogCPM_seurat_list,file = file.path(wdpath,"GeoMX_batchcorrection","LogCPM-Seurat_for_DLBCL_includeTumorMergeMacro-batchcorrected-DEGvalidated.qs"))

LogCPM_seurat_list <- qread(file.path(wdpath,"GeoMX_batchcorrection","LogCPM-Seurat_for_DLBCL_includeTumorMergeMacro-batchcorrected-DEGvalidated.qs"))
Upper_seurat_list <- qread(file.path(wdpath,"GeoMX_batchcorrection","upperquartile-Seurat_for_DLBCL_includeTumorMergeMacro-batchcorrected-DEGvalidated.qs"))
...
for (seurat_name in names(seurat_list)) {
  ...
  if(min(seurat_obj@assays$RNA@counts) < 0 ){next()}
