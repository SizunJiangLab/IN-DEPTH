...
fusion_image = tifffile.imread('../FINAL-TONSIL_2nd_L25_Scan1.qptiff')

fusion_dapi = fusion_image[0]

fusion_rotate = cv2.rotate(fusion_dapi, cv2.ROTATE_180)

fusion_seg = tifffile.imread('../seg_result/L25_MESMER_mask.tiff')

geomx_image = tifffile.imread('../img_registration/20240312_Tonsil_L25.ome.tiff')
...
np.savetxt('../img_registration/L25_affine_matrix.txt', A)

A_inverse = np.linalg.inv(np.vstack((A, [0, 0, 1])))

np.savetxt('../img_registration/L25_inverse_affine_matrix.txt', A_inverse)
...
with tifffile.TiffWriter('../img_registration/L25_SIFT_affline_aligned.ome.tiff', bigtiff=True) as tif:
...
tifffile.imwrite('../img_registration/L25_SIFT_aligned_MESMER_mask.tiff', output_seg)