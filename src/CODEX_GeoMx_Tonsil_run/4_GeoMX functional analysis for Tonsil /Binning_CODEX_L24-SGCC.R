...
for (i in paste0("ROI_",0:15)){
  ...
  for (k in 1:length(cellcombination.name)){
    ...
    p.out <- ggplot(plot_data, aes(x = as.numeric(x_bin), y = as.numeric(y_bin), fill = Color)) +
      ...
      theme_void()
    p.out
    save.name <- paste("L24",ROI_number,CT1,CT2,".tiff",sep = "_")
...
write.csv(SGCC_matrix,file = file.path(wdpath,"SGCC_relevant_analysis","SGCC_matrix_L24.csv"),quote = F)
...
write.csv(Impulse_res_out,
          file = file.path(wdpath, "SGCC_relevant_analysis", paste0("L24", "--", CT.use, "--", group.name, "_Gene.csv")),
          quote = F)
...
qs::qsave(norm_mat_B, file = file.path(wdpath,"SGCC_relevant_analysis","norm_mat_B_L24.qs"))
qs::qsave(norm_mat_T, file = file.path(wdpath,"SGCC_relevant_analysis","norm_mat_T_L24.qs"))
qs::qsave(matrix_merge, file = file.path(wdpath,"SGCC_relevant_analysis","matrix_merge_L24.qs"))
