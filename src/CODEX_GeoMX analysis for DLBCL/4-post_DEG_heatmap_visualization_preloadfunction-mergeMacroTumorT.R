...
mt.seurat_log_mergermacro <- qread(file.path(wdpath,"GeoMX_batchcorrection","LogCPM-Seurat_for_DLBCL_includeTumorMergeMacro-batchcorrected-DEGvalidated.qs"))
CD4T_expr_mat <- mt.seurat_log_mergermacro$Seurat_top5000_k2@assays$RNA@data
Macro_expr_mat <- mt.seurat_log_mergermacro$Seurat_top1000_k3@assays$RNA@data
Tumor_expr_mat <- mt.seurat_log_mergermacro$Seurat_top1000_k3@assays$RNA@data
# 
Seurat_virus <- mt.seurat_log_mergermacro$Seurat_top1000_k3

EBV_expr <- Seurat_virus@assays$RNA@data[EBV_genes, ]
total_viral_expr <- colSums(EBV_expr)
total_gene_expr <- colSums(Seurat_virus@assays$RNA@data)
...
Seurat_virus$CXCL9 <- Seurat_virus@assays$RNA@data["CXCL9",]
Seurat_virus$SPP1 <- Seurat_virus@assays$RNA@data["SPP1",]
Seurat_virus$CXCL9_SPP1 <- Seurat_virus$CXCL9/Seurat_virus$SPP1
Seurat_virus$LMP1_g <- Seurat_virus@assays$RNA@counts["LMP1",]
# subset M1 and M2
...
tumor_meta <- subset(Seurat_virus@meta.data, grepl("Tumor", SegmentLabel))
