...
Macro_customize_M2 <- c( "ARG1", "ARG2", "IL10", "CD32", "CD163", "CD23", "CD200R1",
                         "PDCD1LG2", "CD274", "MARCO", "CSF1R", "CD206", "IL1RN",
                         "IL1R2", "IL4R", "CCL4", "CCL13", "CCL20", "CCL17", "CCL18",
                         "CCL22", "CCL24", "LYVE1", "VEGFA", "VEGFB", "VEGFC", "VEGFD",
                         "EGF", "CTSA", "CTSB", "CSTC", "CTSD", "TGFB1", "TGFB2", "TGFB3",
                         "MMP14", "MMP19", "MMP9", "CLEC7A", "WNT7B", "FASL", "TNFSF12",
                         "TNFSF8", "CD276", "VTCN1", "MSR1", "FN1", "IRF4")
Macro_customize_M1 <- c(  "IL12", "IL23", "TNF", "IL6", "CD86", "IL1B", "MARCO",
                          "NOS2", "CD64", "CD80", "CXCR10", "CXCL9", "CXCL10",
                          "CXCL11", "IL1A", "CCL5", "IRF5", "IRF1", "CD40",
                          "IDO1", "KYNU", "CCR7")
# tumor signatures
...
CD4T_customized_Tcell_exhaustion <- c("CTLA4", "HAVCR2", "LAG3", "PDCD1", "TIGIT", "ENTPD1",
                                      "BTLA", "CD244", "VSIR", "CD160", "NT5E", "ADORA2A", "PVRIG", "SIGLEC7", "SIGLEC9")
...
# virus_genes <- c(
#   "BNRF1.1", "LMP1.1", "EBNA2.2", "EBNA2.1", "LMP1.2",
#   "EBNALP.1", "EBNALP.2", "RPMS1", "EBNA1.2", "EBNA1.1",
#   "EBNA2", "EBNA1", "BZLF1.2", "BALF1.1", "BHRF1.1", "BALF1.2",
#   "RPMS1.1", "BCRF1.1", "BNLF2A", "BALF1", "BCRF1", "BZLF1.1",
#   "BHRF1.2", "BNLF2A.1", "BNLF2A.2", "EBER1.1", "EBER1", "EBER2",
#   "EBER2.1", "BZLF1", "RPMS1.2", "BNRF1.2", "BCRF1.2"
# )
virus_genes <- c(
  "BCRF1", "BCRF1.1", "BCRF1.2",
  "BNLF2A", "BNLF2A.1", "BNLF2A.2",
  "BNLF2B", "BNLF2B.1", "BNLF2B.2",
  "BZLF1", "BZLF1.1", "BZLF1.2",
  "EBNA1", "EBNA1.1", "EBNA1.2",
  "EBNA1BP2",
  "EBNA2", "EBNA2.1", "EBNA2.2",
  "EBNALP", "EBNALP.1", "EBNALP.2",
  "LMP1", "LMP1.1", "LMP1.2",
  "RPMS1", "RPMS1.1", "RPMS1.2",
  "BNRF1.1", "BALF1.1", "BHRF1.1",
  "BALF1.2", "BALF1", "BHRF1.2",
  "EBER1.1", "EBER1", "EBER2", "EBER2.1",
  "BNRF1.2"
)

# Load macrophage signatures from RDS file
macrophage_signatures <- readRDS("/bmbl_data/yuzhou/collaborative/Sizun_lab/INDEPTH/SGCC/SGWT_results/DLBCL_GeoMX/Data/DLBCL_run/macrophage_signature.RDS")
macrophage_signatures <- sapply(macrophage_signatures, function(x) gsub("-",".",x))
# Extract specific macrophage signatures
MoMac_VERSE_cluster_2 <- macrophage_signatures[["MoMac_VERSE_cluster_2"]]
MoMac_VERSE_cluster_3 <- macrophage_signatures[["MoMac_VERSE_cluster_3"]]
MoMac_VERSE_cluster_6 <- macrophage_signatures[["MoMac_VERSE_cluster_6"]]
MoMac_VERSE_cluster_13 <- macrophage_signatures[["MoMac_VERSE_cluster_13"]]
MoMac_VERSE_cluster_16 <- macrophage_signatures[["MoMac_VERSE_cluster_16"]]
MoMac_VERSE_cluster_17 <- macrophage_signatures[["MoMac_VERSE_cluster_17"]]
...
functionalenrichment <- list(
  ...
  Macro_customize_M2 = Macro_customize_M2,
  Macro_customize_M1 = Macro_customize_M1,
  # Tumor
  ...
  # Macrophage signatures from VERSE clusters
  Macro_MoMac_VERSE_cluster_2 = MoMac_VERSE_cluster_2,
  Macro_MoMac_VERSE_cluster_3 = MoMac_VERSE_cluster_3,
  Macro_MoMac_VERSE_cluster_6 = MoMac_VERSE_cluster_6,
  Macro_MoMac_VERSE_cluster_13 = MoMac_VERSE_cluster_13,
  Macro_MoMac_VERSE_cluster_16 = MoMac_VERSE_cluster_16,
  Macro_MoMac_VERSE_cluster_17 = MoMac_VERSE_cluster_17
)

# ============================================================================
# Customized marker panels (preloadable database for benchmarking/lineage)
# These variables are made available globally when sourcing this file
# ============================================================================

CD8T_markers <- c(
  "CD8A", "CD8B", "GZMB", "PRF1", "IFNG", "TNF", "FASLG",
...
B_cellmarkers <- c(
  "CD19", "CD20", "MS4A1", "CD22", "CD79A", "CD79B",
...
Macrophage_marker_list <- list(
  M1_marker = c(
    "IL23", "TNF", "CXCL9", "CXCL10", "CXCL11", "CD86", "IL1A", "IL1B", "IL6", "CCL5",
...
TAM_gene_clusters <- list(
  IFN_TAMs = c(
    "CASP1", "CASP4", "CCL2", "CCL3", "CCL4", "CCL7", "CCL8", "CD274", "CD40",
    "CXCL2", "CXCL3", "CXCL9", "CXCL10", "CXCL11", "IDO1", "IFI6", "IFIT1",
    "IFIT2", "IFIT3", "IFITM1", "IFITM3", "IRF1", "IRF7", "ISG15", "LAMP3",
    "PDCD1LG2", "TNFSF10", "C1QA", "C1QC", "CD38", "IL4I1", "IFI44L"
  ),
  Inflam_TAMs = c(
    "CCL2", "CCL3", "CCL4", "CCL5", "CCL20", "CCL3L1", "CCL3L3", "CCL4L2", "CCL4L4",
    "CXCL1", "CXCL2", "CXCL3", "CXCL5", "CXCL8", "G0S2", "IL1B", "IL1RN", "IL6",
    "INHBA", "KLF2", "KLF6", "NEDD9", "PMAIP1", "S100A8", "S100A9", "SPP1"
  ),
  LA_TAMs = c(
    "ACP5", "AOPE", "APOC1", "ATF1", "C1QA", "C1QB", "C1QC", "CCL18", "CD163",
    "CD36", "CD63", "CHI3L1", "CTSB", "CTSD", "CTSL", "F13A1", "FABP5", "FOLR2",
    "GPNMB", "IRF3", "LGALS3", "LIPA", "LPL", "MACRO", "MERTK", "MMP7", "MMP9",
    "MMP12", "MRC1", "NR1H3", "NRF1", "NUPR1", "PLA2G7", "RNASE1", "SPARC",
    "SPP1", "TFDP2", "TREM2", "ZEB1"
  ),
  Angio_TAMs = c(
    "ADAM8", "AREG", "BNIP3", "CCL2", "CCL4", "CCL20", "CD163", "CD300E", "CD44",
    "CD55", "CEBPB", "CLEC5A", "CTSB", "EREG", "FCN1", "FLT1", "FN1", "HES1",
    "IL1B", "IL1RN", "IL8", "MAF", "MIF", "NR1H3", "OLR1", "PPARG", "S100A8",
    "S100A9", "S100A12", "SERPINB2", "SLC2A1", "SPIC", "SPP1", "THBS1", "TIMP1",
    "VCAN", "VEGFA"
  ),
  Reg_TAM = c(
    "CCL2", "CD274", "CD40", "CD80", "CD86", "CHIT1", "CX3CR1", "HLA.A", "HLA.C",
    "HLA.DQA1", "HLA.DQB1", "HLA.DRA", "HLA.DRB1", "HLA.DRB5", "ICOSLG", "IL10",
    "ITGA4", "LGALS9", "MACRO", "MRC1", "TGFB2"
  ),
  Prolif_TAM = c(
    "CCNA2", "CDC45", "CDK1", "H2AFC", "HIST1H4C", "HMGB1", "HMGN2", "MKI67",
    "RRM2", "STMN1", "TOP2A", "TUBA1B", "TUBB", "TYMS"
  )
)

customized_marker_db <- list(
  CD4T_markers = CD4T_markers,
  CD8T_markers = CD8T_markers,
  B_cellmarkers = B_cellmarkers,
  Macrophage_marker_list = Macrophage_marker_list,
  TAM_gene_clusters = TAM_gene_clusters
)

get_customized_marker_db <- function() {
  customized_marker_db
}
