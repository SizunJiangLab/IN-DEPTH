...
perform_DEA_limma_edgeR <- function(seurat_obj = my.seurat,
  ...
  # previous script produce counts as normalized data
  expr_matrix <- GetAssayData(object = seurat_obj,layer = "counts")

  # Extract metadata (for grouping, EBV, and confounder information)
  metadata <- seurat_obj@meta.data

  # Subset data by EBV status and handle target vs rest comparison
  group <- metadata[[group_column]]
  EBV <- metadata[[EBV_column]]

  # Handle "target vs rest" comparison
  if (ident2 == "REST") {
    # Keep only cells that match the EBV status
    ...
    # Create binary grouping: target cell type vs all others
  ...
  if (!is.null(confounder_columns)) {
    ...
    # Build a minimal design data frame and then create the model matrix
    if (consider_CTnumber == T) {
  ...
  # Choose analysis path based on input slot
  if (tolower(assay) == "counts") {
    # Count-based workflow with voom
    dge <- DGEList(counts = expr_matrix, group = group)
    v <- voom(dge, design, plot = TRUE)
    fit <- lmFit(v, design)
    fit <- eBayes(fit)
  } else {
    # Already normalized/log-scale data: use limma directly with trend moderation
    fit <- lmFit(expr_matrix, design)
    fit <- eBayes(fit, trend = TRUE)
  }

  # Capture group levels for clear labeling
  baseline_level <- levels(group)[1]
  contrast_level <- levels(group)[2]

  # Select the group coefficient by name to avoid relying on column index
